	...
	switch(t->key) {
		case KEY_INVERT:
			...
			if (strcmp(t -> value -> cstring, "on") == 0) {
				...
				text_layer_set_background_color(line_date_batt.s_anim_curr_layer, GColorWhite);
				text_layer_set_text_color(line_date_batt.s_anim_curr_layer, GColorBlack);
				text_layer_set_background_color(line_date_batt.s_anim_next_layer, GColorWhite);
				text_layer_set_text_color(line_date_batt.s_anim_next_layer, GColorBlack);
			...
			break;
		case KEY_SHOW_DATE_BATT:
			//it's the KEY_SHOW_DATE key
			if (strcmp(t -> value -> cstring, "on") == 0) {
				//set and save show date and battery
				//restart animations
				//updateLineDateToBatt(&line_date_batt);
				//animation_schedule((Animation *)line_date_batt.s_curr_animation);
				//animation_schedule((Animation *)line_date_batt.s_next_animation);
				//show date and battery layer
				layer_set_hidden((Layer *) line_date_batt.s_anim_curr_layer, false);
				layer_set_hidden((Layer *) line_date_batt.s_anim_next_layer, false);
				...
				//save show date
				persist_write_bool(KEY_SHOW_DATE_BATT, true);
			} else if (strcmp(t -> value -> cstring, "off") == 0) {
				//set and save hide date and battery
				//stop any animation in progress
				//animation_unschedule_all();
				//hide date and battery layer
				layer_set_hidden((Layer *) line_date_batt.s_anim_curr_layer, true);
				layer_set_hidden((Layer *) line_date_batt.s_anim_next_layer, true);
				//set new position for hours and minutes layer
				layer_set_frame((Layer *) s_hours_layer, GRect(0,10,144,70));
				layer_set_frame((Layer *) s_minutes_layer, GRect(0,80,144,70));
				//save hide date
				persist_write_bool(KEY_SHOW_DATE_BATT, false);
			} else {
