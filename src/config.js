...
Pebble.addEventListener("showConfiguration", function() {
	...
	Pebble.openURL('http://flashback2k14.github.io/yeahclockforpebble/dev/index.html?' + encodeURIComponent(JSON.stringify(options)));
});
  ...
  if (e.response.charAt(0) === "{" && e.response.slice(-1) === "}" && e.response.length > 5) {
    options = JSON.parse(decodeURIComponent(e.response));
		...
		options = {
			...
			"KEY_SWITCH_TEMPERATURE": options.selectTemperature
		};
